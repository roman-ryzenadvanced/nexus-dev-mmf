...
describe.skipIf(!process.env.OPENAI_API_KEY)('OpenAI — real API smoke', () => {
  it(
    ...
    async () => {
      ...
      const res = await provider.chat([userMsg('Reply with exactly: pong')], {
        model: 'gpt-4o-mini',
      });
  ...
  it(
    ...
    async () => {
      ...
      const gen = provider.streamChat!([userMsg('Count from 1 to 5, separated by spaces.')], { model: 'gpt-4o-mini' });
      const deltas: string[] = [];
      let final;
      while (true) {
...
describe.skipIf(!process.env.ANTHROPIC_API_KEY)('Anthropic — real API smoke', () => {
  it(
    'returns a chat completion with system prompt',
    async () => {
  ...
  it(
    ...
    async () => {
      ...
      const gen = provider.streamChat!([userMsg('Count from 1 to 5.')], {
        model: 'claude-3-5-haiku-20241022',
      });
      const deltas: string[] = [];
      let final;
...
describe.skipIf(!process.env.ZAI_API_KEY)('Z.ai — real API smoke', () => {
  it(
    'returns a chat completion (direct, no MMFE)',
    async () => {
  ...
  it(
    'returns builtin models from fetchModels()',
    async () => {
      const provider = new ZAIProvider({ apiKey: process.env.ZAI_API_KEY });
      const models = await provider.fetchModels();
