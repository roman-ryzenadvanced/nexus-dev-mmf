...
function makeConfig(): AppConfig {
  return {
    ...
    useMMFE: false,
    providers: [
      { id: 'fake', kind: 'openai', name: 'Fake', mmfe: false, defaultModel: 'gpt-test' },
    ],
    manualModels: [],
    mcpServers: [],
...
describe('orchestrator tool-call execution loop', () => {
  ...
  it('returns response immediately when no tool calls', async () => {
    ...
    const res = await sendChat(makeConfig(), providers, messages, { toolRegistry, onToolCall });
  ...
  it('executes a single tool call and continues', async () => {
    ...
    const provider = new FakeToolCallingProvider([
      { content: '', toolCalls: [{ id: 'tc1', name: 'get_weather', args: { city: 'NYC' }, status: 'pending' }] },
      { content: 'The weather in NYC is sunny.' },
    ]);

    const providers = new Map([['fake', provider as unknown as Provider]]);
    const messages: ChatMessage[] = [
      { id: 'u1', role: 'user', content: "What's the weather in NYC?", ts: Date.now() },
    ];
  ...
  it('chains multiple tool rounds', async () => {
    ...
    const provider = new FakeToolCallingProvider([
      { content: '', toolCalls: [{ id: 'a', name: 'next', args: {}, status: 'pending' }] },
      { content: '', toolCalls: [{ id: 'b', name: 'next', args: {}, status: 'pending' }] },
      { content: '', toolCalls: [{ id: 'c', name: 'next', args: {}, status: 'pending' }] },
      { content: 'Done. Counted to 3.' },
    ]);

    const providers = new Map([['fake', provider as unknown as Provider]]);
    const messages: ChatMessage[] = [
      { id: 'u1', role: 'user', content: 'count to 3', ts: Date.now() },
    ];
  ...
  it('stops at maxToolRounds even if the model keeps calling tools', async () => {
    ...
    const providers = new Map([['fake', provider as unknown as Provider]]);
    const messages: ChatMessage[] = [
      { id: 'u1', role: 'user', content: 'go', ts: Date.now() },
    ];
  ...
  it('surfaces tool handler errors as status=error', async () => {
    ...
    const provider = new FakeToolCallingProvider([
      { content: '', toolCalls: [{ id: 'b1', name: 'boom', args: {}, status: 'pending' }] },
      { content: 'Sorry, the tool failed.' },
    ]);

    const providers = new Map([['fake', provider as unknown as Provider]]);
    const messages: ChatMessage[] = [
      { id: 'u1', role: 'user', content: 'run boom', ts: Date.now() },
    ];
  ...
  it('does not execute tools when no toolRegistry is provided', async () => {
    const provider = new FakeToolCallingProvider([
      { content: '', toolCalls: [{ id: 'x', name: 'unreachable', args: {}, status: 'pending' }] },
    ]);
