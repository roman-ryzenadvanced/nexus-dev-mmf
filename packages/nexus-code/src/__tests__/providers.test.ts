...
describe('OpenAIProvider — request construction + response parsing', () => {
  ...
  beforeEach(() => {
    ...
    (
      provider as unknown as {
        client: { chat: { completions: { create: typeof chatCreateMock } } };
      }
  ...
  it('includes tools when provided', async () => {
    ...
    const messages: ChatMessage[] = [{ id: 'u1', role: 'user', content: 'what time is it?', ts: Date.now() }];
  ...
  it('throws ProviderError on API failure', async () => {
    chatCreateMock.mockRejectedValueOnce(Object.assign(new Error('Rate limited'), { status: 429 }));
  ...
  it('fetchModels calls /v1/models', async () => {
    ...
    (provider as unknown as { client: { models: { list: typeof listMock } } }).client.models.list = listMock;
...
describe('AnthropicProvider — request construction + response parsing', () => {
  ...
  beforeEach(() => {
    ...
    (
      provider as unknown as {
        client: { messages: { create: typeof messagesCreateMock } };
      }
  ...
  it('includes tools with input_schema.type=object', async () => {
    ...
    await provider.chat([{ id: 'u', role: 'user', content: 'what time?', ts: Date.now() }], {
      model: 'claude-3-5-sonnet-20241022',
      tools: [
        {
          name: 'get_time',
  ...
  it('parses tool_use blocks from response', async () => {
    ...
    const res = await provider.chat([{ id: 'u', role: 'user', content: 'time?', ts: Date.now() }], { model: 'claude-3-5-sonnet-20241022' });
  ...
  it('fetchModels calls /v1/models with x-api-key + anthropic-version headers', async () => {
    fetchMock.mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          data: [{ id: 'claude-3-5-sonnet-20241022' }, { id: 'claude-3-opus-20240229', display_name: 'Claude 3 Opus' }],
        }),
  ...
  it('throws ProviderError when apiKey is missing', async () => {
    ...
    await expect(noKey.chat([{ id: 'u', role: 'user', content: 'hi', ts: Date.now() }])).rejects.toThrow('missing an API key');
  });
});
